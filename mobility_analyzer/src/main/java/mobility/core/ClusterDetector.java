...
//		listUsers.add(userService.findUserById(new Long(20094532)));
		listUsers.addAll(userService.findAllSelectedUsers(1));
	...
	private static void processClurteredPoints(){
		...
		for(ClusteredUser clusteredUser : listClusteredUsers){
			...
			for(Cluster<DoublePoint> cluster : clusteredUser.getCluster()){
				...
				if(cluster.getPoints().size() >= 3){ //POI usa 2 e AC usa 3
					++countCluster;
...
//		DBSCANClusterer dbscan = new DBSCANClusterer(40.0, 2, new GeoDistance()); // POI
		DBSCANClusterer dbscan = new DBSCANClusterer(40.0, 3, new GeoDistance()); // AC
